...

  {
    ...
    items: [
      ...
      },
      {
        ...
        a: '<p>Yes. Copy an image, then paste it into the chat box however your device pastes anything else — Ctrl + V on Windows or Linux, Cmd + V on a Mac, or a long press and <b>Paste</b> on a phone or tablet. It attaches exactly as an uploaded photo does and is read the same way: pasting is a shortcut into Snap &amp; Solve, not a separate feature.</p><p>Where the picture came from does not matter. A screen capture works — the Windows Snipping Tool (Win + Shift + S) is one example, and every system has its own — and so does right-clicking or long-pressing a picture in your browser and choosing <b>Copy Image</b>.</p><p>One piece of browser behaviour is worth knowing, because it is the one case that looks broken and is not. <b>Selecting</b> an image on a page and copying does not put the picture on your clipboard — browsers copy the surrounding text instead, so there is nothing for the chat to attach. Choose <b>Copy Image</b> when you want the picture itself.</p><p>Text and images travel together. Copying a passage from a PDF that contains both will paste the text into your message and attach the picture at the same time.</p><p>On a phone or tablet the camera and gallery buttons in the chat are the most direct route, and they work in every browser — whether or not that browser offers an image paste.</p>',
      },
      {
