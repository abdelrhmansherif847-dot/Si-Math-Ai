...
t.section('F-5 — a suspended context is resumed when the layer is re-enabled');

/* WHY THIS SECTION NEEDS ITS OWN SANDBOX. The harness at the top of this file
   supplies NO AudioContext, so context() returns null and enable() returns
   false — which is why every assertion above passes without the layer ever
   having run. That blind spot is exactly what hid F-5: disable() suspends the
   context and, before the fix, nothing ever resumed it, so a second enable()
   armed the whole schedule against a silent destination and reported success.

   The fake records suspend/resume rather than simulating audio. State is the
   observable that matters — a re-enabled layer whose context is still
   'suspended' plays nothing — and a call log is what tells a real fix apart
   from a state assignment that merely looks like one. */
function withAudio(initialState, opts) {
  const calls = [];
  const ctx = {
    state: initialState,
    destination: {},
    ...
    decodeAudioData: () => Promise.reject(new Error('no decode in the sandbox')),
    suspend() { calls.push('suspend'); ctx.state = 'suspended'; return Promise.resolve(); },
  };
  // An engine with no resume() at all — the `&& ctx.resume` half of the guard.
  if (!(opts && opts.noResume)) {
    ctx.resume = () => { calls.push('resume'); ctx.state = 'running'; return Promise.resolve(); };
  }
  // A constructor returning an object yields that object, so `new AC()` is ctx.
  function AC() { return ctx; }
  const r = {};
  evalSnippet(read('exam-ambience.js'), {
    globalThis: r, window: r, AudioContext: AC,
    setTimeout: () => 0, clearTimeout: () => {},
    fetch: () => Promise.reject(new Error('no assets in the sandbox')),
  }, []);
  r.AudioContext = AC;
  r.setTimeout = () => 0;
  r.clearTimeout = () => {};
  r.fetch = () => Promise.reject(new Error('no assets in the sandbox'));
  return { A: r.SiExamAmbience, ctx, calls };
}

// A context constructed outside a user gesture is BORN suspended. Before the
// fix this was silence on the very first enable(), not just on the second.
{
  const { A: B, ctx, calls } = withAudio('suspended');
  t.ok('enable() reports success on a born-suspended context', B.enable() === true);
  t.ok('and the layer is on', B.isOn() === true);
  t.is('the context is running, not suspended', ctx.state, 'running');
  t.is('resume was called exactly once', calls.filter((c) => c === 'resume').length, 1);
}

// The round trip F-5 is named for.
{
  const { A: B, ctx, calls } = withAudio('running');
  B.enable();
  t.is('a context already running is not resumed', calls.filter((c) => c === 'resume').length, 0);
  t.ok('disable() reports success', B.disable() === true);
  t.is('disable suspends the context', ctx.state, 'suspended');
  t.is('suspend was called once', calls.filter((c) => c === 'suspend').length, 1);

  t.ok('re-enable reports success', B.enable() === true);
  t.ok('and the layer is on again', B.isOn() === true);
  t.is('THE FIX: the context is running again after disable then enable',
    ctx.state, 'running');
  t.is('resume was called on the re-enable', calls.filter((c) => c === 'resume').length, 1);
  t.is('the whole call sequence', calls, ['suspend', 'resume']);
}

// enable() while already on is an early return, so it must not resume either —
// the fix is meant to be reached on a real transition and nowhere else.
{
  const { A: B, calls } = withAudio('running');
  B.enable(); B.enable(); B.enable();
  t.is('a repeated enable resumes nothing', calls.length, 0);
}

// The guard is `ctx.state === 'suspended' && ctx.resume`. An engine without
// resume() must degrade to silence, never to an exception that takes the
// layer — and the whole exam — down with it.
{
  const { A: B, ctx } = withAudio('suspended', { noResume: true });
  t.ok('an engine with no resume() does not throw', B.enable() === true);
  t.is('it simply stays suspended', ctx.state, 'suspended');
}

// The fix must not have moved the level or the schedule: both are measured
// numbers, and this section is the only thing in the file that touches enable().
{
  const { A: B } = withAudio('running');
  t.is('the trims are unchanged by the fix', B.trim, A.trim);
  t.is('the peaks are unchanged by the fix', B.peakDbfs, A.peakDbfs);
  t.is('the durations are unchanged by the fix', B.durations, A.durations);
  t.is('the schedule is unchanged by the fix',
    B.schedule(35, 0).map((e) => e.clock + ' ' + e.clip),
    A.schedule(35, 0).map((e) => e.clock + ' ' + e.clip));
}
