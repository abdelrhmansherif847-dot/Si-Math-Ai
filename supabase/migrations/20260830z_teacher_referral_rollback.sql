...
-- ===========================================================================
-- STATUS: PREPARED — NOT APPLIED. The undo for 20260830a and 20260830b.
--
-- Run this ONLY to reverse the referral feature. It restores
-- approve_payment_request to its pre-hook definition first, so the payment path
-- stops calling award_referral_commission before that function is dropped —
-- the other order leaves every payment approval raising "function does not
-- exist" until the transaction commits.
--
-- Dropping referral_commissions destroys accrued commission history. Export it
-- first if any commission has ever been awarded.
-- ===========================================================================

begin;

-- 1. Unhook the payment path FIRST. This is the pre-20260830b definition.
...
-- 2. Now the referral objects can go.
drop function if exists public.award_referral_commission(uuid, uuid, text, numeric);
