...
--
-- 20260901z could restore its prior state byte for byte. This cannot, and the
-- difference is worth stating plainly rather than filing both under "rollback".
--
...
--     trivial at today's 2 rows and not trivial after a year of history;
--   * a drop and recreate of a type that other objects may have come to
--     depend on by the time anyone reads this.
--
-- PRECONDITIONS — check all three before running, because the script will not
-- be able to check the third for you:
--
--   1. No row in workspace_audit_log holds one of the eight new labels. The
--      cast in step 4 fails if one does, which is correct: a label that
...
--      type. Verified empty on 2026-09-01 — one column, no default, no index
--      on it, no function taking or returning it — but re-verify.
--   3. ⚠️ 3b–3g HAVE NOT LANDED. Once 3c/3d write these labels, this file
--      stops being a rollback of 3a and becomes a rollback of everything
--      built on it. The window in which this script is meaningful closes the
--      moment the first exam RPC ships.
--
-- If you are here to undo a MISTAKE IN THE LABEL SET rather than to undo 3a
...
begin
  ...
  if v_used is not null then
    raise exception
      'rollback 3a refused: the audit log already records %. Removing these labels '
...
-- 3 · destroy and rebuild the type with exactly the original eight, in order
...
begin
  ...
  if v_labels is distinct from v_expected then
    raise exception E'rollback 3a: labels not restored\n  want: %\n  got : %', v_expected, v_labels;
  end if;
  ...
  if v_type is distinct from 'workspace_audit_action' then
    raise exception 'rollback 3a: action column is %, not workspace_audit_action', v_type;
  end if;
end $$;
