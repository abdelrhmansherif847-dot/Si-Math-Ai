...
(function (root) {
  ...
  function enable() {
    try {
      ...
      /* RESUME A SUSPENDED CONTEXT. disable() suspends it, and a context
         constructed outside a user gesture is born suspended — so without this
         a second enable() armed the whole schedule against a silent
         destination and reported success. Nothing else called resume(): before
         this line the module referenced ctx.state exactly once, in disable().
         Deliberately not awaited, for the same reason nothing else here is:
         events are armed against the module clock, not against the resume, and
         a rejected promise must not take the layer down. */
