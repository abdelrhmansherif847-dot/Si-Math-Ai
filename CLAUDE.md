...
| CI | `node tests/run-all.mjs` — **84 checks** (counted 2026-09-05, after the DSAT question knowledge base added `dsat-kb`, `dsat-kb-registry` and `validate-dsat-kb`; the exam-knowledge ingestions had added `validate-exam-knowledge` and `validate-exam-kdg`; the Stage-24/25 capacity and corpus work had taken it from 74 to 79; Stage 3.5 added `est-form-gates`; Stage 2.5 had added `est-assembly-gates` and the `validate-est-assembly` gate; Stage 2 had added `est-compose`, `est-blueprint-gates` and `validate-est-signatures`, Stage 1 `est-primitives`, `est-fingerprint` and `validate-est-primitives`). Plus a SECOND GitHub job, `visual`, which installs Chromium and runs `scripts/check-visual-fidelity.cjs` — the pixel comparison. **That job has never run on GitHub**: it was written on a branch and CI here only runs on `main` and pull requests |
