...
| CI | `node tests/run-all.mjs` — **68 checks** (counted 2026-09-01, after Stage 1 added `est-primitives`, `est-fingerprint` and the `validate-est-primitives` gate). Plus a SECOND GitHub job, `visual`, which installs Chromium and runs `scripts/check-visual-fidelity.cjs` — the pixel comparison. **That job has never run on GitHub**: it was written on a branch and CI here only runs on `main` and pull requests |
